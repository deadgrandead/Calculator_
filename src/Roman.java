enum Roman {
    ...
    public static Roman fromInt(int number) {
        if (number < 1) {
            throw new IllegalArgumentException("Римские числа должны быть больше 0");
        }
        StringBuilder result = new StringBuilder();
        int remaining = number;
        while (remaining > 0) {
            Roman current = valueOf(remaining);
            result.append(current.name());
            remaining -= current.GetValue();
        }
        return Roman.valueOf(result.toString());
    }
}
